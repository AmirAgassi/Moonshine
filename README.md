...
**"GameExecutableNames"** contains a list of game executable names. If the script doesn't detect one of your games, add the executable name to this list. Be careful, as the script checks if there is any match between a process on your PC and the entry in the list. If you'd enter a game as "wizard" instead of a more specific or executable (name.exe) name, some program like an install wizard might interfere and wrongly suspend your miner.
